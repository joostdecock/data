...
class DraftController 
{
    ...
    {
        $scale = 3.54330709;
        $svg = new \SimpleXMLElement($svgstr);

        // Patch SVG attributes
        foreach($svg->attributes() as $key => $val) {
            if($key == 'width') $width = round($val/$scale);
            else if($key == 'height') $height = round($val/$scale);
            $keys[] = $key;
        }
        foreach($keys as $key) unset($svg[$key]);
        $svg->addAttribute("width", $width.'mm');
        $svg->addAttribute("height", $height.'mm');
        $svg->addAttribute("viewBox", "0 0 $width $height");
        $svg->addAttribute("freesewing:version", "unknown, predates v1.3.0");
        $svg->addAttribute("freesewing:origin", $this->container['settings']['app']['origin']);

        // Remove scale wrapper
        unset($svg->g[0]['transform']);

        return $svg->asXML();
    } 
        
    private function loadUnpatchedDrafts($limit=false)
    {
        $db = $this->container->get('db');
        $sql = "SELECT `id` from `drafts` WHERE `drafts`.`data` NOT LIKE '%\"version\":%'";
        if($limit !== false && is_int($limit)) $sql .= " LIMIT $limit";
