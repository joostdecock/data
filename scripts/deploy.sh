...
if [ -z "$TRAVIS_PULL_REQUEST" ]; then
    ...
    if [ "$TRAVIS_BRANCH" = "develop" ] || [ "$TRAVIS_BRANCH" = "master" ] || [ "$TRAVIS_BRANCH" == "v2" ]; then
        if [ "$TRAVIS_PHP_VERSION" = "7.0.29" ]; then
        ...
        echo "Branch is not one of master/develop/v2, not deploying."
    fi
fi
echo "Bye"
